  ...
  async function loadData() {
    ...
    try {
      const [contracts, customers, opportunities] = await Promise.all([
        getJson<ContractRow[]>('/contracts?limit=50'),
        getJson<CustomerRow[]>('/customers'),
      ...
      if (!selectedContractId && contracts.data[0]) {
  ...
  async function loadContractDetail(id: number) {
    try {
      const [contract, contractMilestones] = await Promise.all([
        ...
        getJson<MilestoneRow[]>(`/contracts/${id}/milestones`)
      ]);

      setSelectedContract(contract.data);
      setMilestones(contractMilestones.data);
