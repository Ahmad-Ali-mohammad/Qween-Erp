...
export default function App() {
  const rawSystemBase = typeof import.meta !== 'undefined' ? (import.meta as any).env?.VITE_SYSTEM_BASE_URL ?? '' : '';
  const systemBase = rawSystemBase.replace(/\/$/, '');
  const resolveSystemHref = (routeBase: string) => (systemBase ? `${systemBase}${routeBase}` : routeBase);
      ...
      >
        ...
        <SectionCard title={pickLocalized(locale, 'كتالوج الأنظمة', 'System Catalog')} eyebrow="Apps">
          <div className="ui-grid">
            {systems.map((system) => (
              <SystemCard key={system.key} system={system} locale={locale} href={resolveSystemHref(system.routeBase)} />
            ))}
          </div>
        </SectionCard>
      </AppShell>
          ...
          <div className="ui-grid">
            {group.items.map((system) => (
              <SystemCard
                ...
                system={system}
                locale={locale}
                href={resolveSystemHref(system.routeBase)}
                meta={
                  <>
