  ...
  async function loadData() {
    ...
    try {
      const [customersRes, opportunitiesRes, contractsRes] = await Promise.all([
        getJson<CustomerRow[] | RowsEnvelope<CustomerRow>>('/crm/customers?limit=200'),
        getJson<OpportunityRow[] | RowsEnvelope<OpportunityRow>>('/crm/opportunities?limit=50'),
        getJson<ContractRow[] | RowsEnvelope<ContractRow>>('/crm/contracts?limit=50')
      ]);
      const normalizedOpportunities = normalizeRows(opportunitiesRes.data);
      ...
      setSelectedOpportunityId((current) => current ?? normalizedOpportunities[0]?.id ?? null);
