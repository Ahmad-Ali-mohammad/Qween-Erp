  ...
  async function loadData() {
    ...
    try {
      const [assetsRes, projectsRes, allocationsRes, maintenanceRes] = await Promise.all([
        getJson<AssetRow[]>('/equipment/assets'),
        getJson<ProjectRow[]>('/projects?limit=100'),
        getJson<AllocationRow[]>('/equipment/allocations?limit=50'),
        getJson<MaintenanceRow[]>('/equipment/maintenance?limit=50')
      ]);
      ...
      setSelectedAllocationId((current) => current ?? allocationsRes.data[0]?.id ?? null);
      setSelectedMaintenanceId((current) => current ?? maintenanceRes.data[0]?.id ?? null);
    } catch (loadError) {
