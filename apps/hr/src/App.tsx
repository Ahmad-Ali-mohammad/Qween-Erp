  ...
  async function loadData() {
    ...
    try {
      const [employeesRes, leavesRes, attendanceRes, timesheetsRes, payrollRes] = await Promise.all([
        getJson<EmployeeRow[]>('/hr/employees?limit=50'),
        getJson<LeaveRow[]>('/hr/leaves?limit=50'),
        getJson<AttendanceRow[]>('/hr/attendance?limit=50'),
        getJson<TimesheetRow[]>('/hr/timesheets?limit=50'),
        getJson<PayrollRunRow[]>('/hr/payroll?limit=20')
      ]);
      ...
      setSelectedLeaveId((current) => current ?? leavesRes.data[0]?.id ?? null);
    } catch (loadError) {
