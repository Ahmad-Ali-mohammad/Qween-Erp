  ...
  async function loadData() {
    ...
    try {
      const [requestsRes, ordersRes, receiptsRes, invoicesRes, suppliersRes, projectsRes] = await Promise.all([
        getJson<PurchaseRequestRow[] | RowsEnvelope<PurchaseRequestRow>>('/procurement/requests?limit=50'),
        getJson<PurchaseOrderRow[] | RowsEnvelope<PurchaseOrderRow>>('/procurement/orders?limit=50'),
        getJson<ReceiptRow[] | RowsEnvelope<ReceiptRow>>('/procurement/receipts?limit=50'),
        getJson<VendorInvoiceRow[] | RowsEnvelope<VendorInvoiceRow>>('/procurement/vendor-invoices?limit=50'),
        getJson<SupplierRow[] | RowsEnvelope<SupplierRow>>('/suppliers'),
        getJson<ProjectRow[] | RowsEnvelope<ProjectRow>>('/projects?limit=100')
      ]);

      const normalizedRequests = normalizeRows(requestsRes.data);
      const normalizedOrders = normalizeRows(ordersRes.data);

      setRequests(normalizedRequests);
      setOrders(normalizedOrders);
      setReceipts(normalizeRows(receiptsRes.data));
      setVendorInvoices(normalizeRows(invoicesRes.data));
      setSuppliers(normalizeRows(suppliersRes.data));
      setProjects(normalizeRows(projectsRes.data));
      setSelectedRequestId((current) => current ?? normalizedRequests[0]?.id ?? null);
    } catch (loadError) {
