...
export default function App() {
  ...
  async function loadProjects() {
    ...
    setError(null);
    try {
      const res = await getJson<ProjectRow[] | RowsEnvelope<ProjectRow>>('/projects?limit=50');
  ...
  async function loadProjectDetails(projectId: number) {
    ...
    try {
      const [summaryRes, phasesRes, expensesRes] = await Promise.all([
        ...
        getJson<ExpenseRow[] | RowsEnvelope<ExpenseRow>>(`/projects/${projectId}/expenses?limit=50`)
      ]);
      setSummary(summaryRes.data);
      setPhases(phasesRes.data);
      setExpenses(normalizeRows(expensesRes.data));
    } catch (loadError) {
