  ...
  async function loadData() {
    ...
    try {
      const [inspectionRes, incidentsRes, reportRes, refsRes] = await Promise.all([
        getJson<InspectionRow[] | RowsEnvelope<InspectionRow>>('/quality/inspections?limit=20'),
        getJson<IncidentRow[] | RowsEnvelope<IncidentRow>>('/safety/incidents?limit=30'),
        getJson<SafetyReport>('/safety/reports'),
        getJson<ReferenceData>('/site/reference-data')
      ]);

      const normalizedInspections = normalizeRows(inspectionRes.data);
      const normalizedIncidents = normalizeRows(incidentsRes.data);
      ...
      const openIncident = normalizedIncidents.find((row) => row.status !== 'RESOLVED');
      setSelectedIncidentId(openIncident?.id ?? normalizedIncidents[0]?.id ?? null);
    } catch (loadError) {
      setError(
