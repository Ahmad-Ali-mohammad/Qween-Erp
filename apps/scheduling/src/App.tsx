...
export default function App() {
  ...
  async function loadProjects() {
    const projectsRes = await getJson<ProjectRow[]>('/projects?limit=50');
    setProjects(projectsRes.data);
    if (!selectedProjectId && projectsRes.data[0]) {
  ...
  async function loadProjectData(projectId: number) {
    const [ganttRes, criticalRes, progressRes] = await Promise.all([
      ...
      getJson<ProgressRow[]>(`/scheduling/projects/${projectId}/progress?limit=20`)
    ]);
