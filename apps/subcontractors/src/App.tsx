  ...
  async function loadData() {
    ...
    try {
      const [subsRes, contractsRes, certificatesRes, paymentsRes, reportRes] = await Promise.all([
        getJson<SubcontractorRow[]>('/subcontractors?limit=50'),
        getJson<ContractRow[]>('/subcontractors/contracts?limit=50'),
        getJson<CertificateRow[]>('/subcontractors/certificates?limit=50'),
        getJson<PaymentRow[]>('/subcontractors/payments?limit=50'),
      ...
      setSelectedSubcontractorId((current) => current ?? subsRes.data[0]?.id ?? null);
      setSelectedContractId((current) => current ?? contractsRes.data[0]?.id ?? null);
      setSelectedCertificateId((current) => current ?? certificatesRes.data[0]?.id ?? null);
    } catch (loadError) {
