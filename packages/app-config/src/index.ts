import type { SystemDefinition } from '@erp-qween/domain-types';

export const SYSTEM_APPS: SystemDefinition[] = [
  {
    key: 'control-center',
...
export const SYSTEM_GROUP_ORDER = ['core', 'operations', 'support', 'advanced'] as const;

export function getSystemByKey(key: string) {
  return SYSTEM_APPS.find((system) => system.key === key);
}

export function groupSystems() {
  return SYSTEM_GROUP_ORDER.map((group) => ({
    group,
    systems: SYSTEM_APPS.filter((system) => system.group === group)
  }));
}
