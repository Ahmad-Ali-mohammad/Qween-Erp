...
router.get('/dashboard/charts/sales', async (req: Request, res: Response) => {
  ...
  ok(res, Array.from(buckets.entries()).map(([period, amount]) => ({ period, amount })));
});
...
router.get('/dashboard/recent-transactions', async (_req: Request, res: Response) => {
  const [journals, invoices, payments] = await Promise.all([
    ...
    prisma.invoice.findMany({ take: 10, orderBy: [{ date: 'desc' }, { id: 'desc' }] }),
...
router.get('/notifications/count', async (req: any, res: Response) => {
  ...
  const unread = await prisma.notification.count({ where: { OR: [{ userId }, { userId: null }], isRead: false } });
  ok(res, { unread });
});
...
// Additional administration/report/tax/currency compatibility
router.get('/settings/sequences', async (_req: Request, res: Response) => {
...
router.put('/roles/:id/permissions', async (req: Request, res: Response) => {
  const id = parsePositiveInt(req.params.id, 'roleId');
  const row = await prisma.role.update({ where: { id }, data: { permissions: req.body ?? {} } });
  ok(res, row.permissions ?? {});
});

router.get('/roles/:id', async (req: Request, res: Response) => {
  const id = parsePositiveInt(req.params.id, 'roleId');
  ok(res, await prisma.role.findUnique({ where: { id } }));
});
...
router.put('/integrations/:name/settings', async (req: Request, res: Response) => {
