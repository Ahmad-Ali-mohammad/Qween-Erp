...
import { calculateProjectCostSummary } from '../projects/service';
...
function paymentTypeLabel(type: string) {
  return type === 'RECEIPT' ? 'سند قبض' : type === 'PAYMENT' ? 'سند صرف' : type;
}
...
function stockMovementLabel(type: string) {
  switch (type) {
    case 'ISSUE':
      return 'إذن صرف مخزن';
    case 'RECEIPT':
      return 'إذن استلام مخزن';
    case 'TRANSFER':
      return 'تحويل مخزني';
    case 'ADJUSTMENT':
      return 'تسوية مخزنية';
...
async function buildProjectContext(recordId: number) {
  const [project, company, summary] = await Promise.all([
    ...
    buildCompanyContext(),
    calculateProjectCostSummary(recordId)
  ]);
  ...
  return {
    ...
    summary: {
      totalBudgetDisplay: money(summary.summary.totalBudgetWithChanges),
      actualCostDisplay: money(summary.summary.actualCost),
      varianceDisplay: money(summary.summary.budgetVariance)
    },
    phases: summary.phases.map((phase) => ({
      id: phase.id,
      name: phase.nameAr,
      status: phase.status,
      budgetDisplay: money(phase.budget + phase.approvedChangeOrders),
      actualCostDisplay: money(phase.actualCost)
    })),
...
async function buildPaymentContext(recordId: number) {
  ...
  const partyName =
    payment.type === 'RECEIPT'
      ? payment.customer?.nameEn || payment.customer?.nameAr || 'Customer'
      : payment.supplier?.nameEn || payment.supplier?.nameAr || 'Supplier';
  ...
  return {
    ...
    branch: payment.branch
      ? { id: payment.branch.id, code: payment.branch.code, nameAr: payment.branch.nameAr }
      : { id: null, code: '', nameAr: 'Unassigned branch' },
    party: { name: partyName },
    bank: payment.bank
      ? { name: payment.bank.name, accountNumber: payment.bank.accountNumber, bankName: payment.bank.bankName }
    ...
    document: {
      ...
      dateDisplay: dateLabel(payment.date),
    ...
    allocations: payment.allocations.map((alloc) => ({
      invoiceNumber: alloc.invoice?.number ?? `INV-${alloc.invoiceId}`,
      amountDisplay: money(alloc.amount)
    })),
    generatedAt: dateTimeLabel(new Date())
  };
}

async function buildJournalEntryContext(recordId: number) {
  ...
  if (!entry) throw Errors.notFound('Journal entry not found');
  ...
  return {
    ...
    company,
    branch: entry.branch
      ? { id: entry.branch.id, code: entry.branch.code, nameAr: entry.branch.nameAr }
      : { id: null, code: '', nameAr: 'Unassigned branch' },
    document: {
      id: entry.id,
      ...
      totalDebitDisplay: money(entry.totalDebit),
      totalCreditDisplay: money(entry.totalCredit)
    },
    lines: entry.lines.map((line) => ({
      lineNumber: line.lineNumber,
      accountLabel: `${line.account.code} - ${line.account.nameEn || line.account.nameAr}`,
      description: line.description ?? '',
      debitDisplay: money(line.debit),
      creditDisplay: money(line.credit)
    })),
    generatedAt: dateTimeLabel(new Date())
  };
}

async function buildStockMovementContext(recordId: number) {
  const [movement, company] = await Promise.all([
  ...
  if (!movement) throw Errors.notFound('Stock movement not found');

  const [item, warehouse] = await Promise.all([
    prisma.item.findUnique({ where: { id: movement.itemId } }),
    prisma.warehouse.findUnique({ where: { id: movement.warehouseId } })
  ]);
  ...
  return {
    ...
    branch: movement.branch
      ? { id: movement.branch.id, code: movement.branch.code, nameAr: movement.branch.nameAr }
      : { id: null, code: '', nameAr: 'Unassigned branch' },
    item: { name: item?.nameEn || item?.nameAr || `Item ${movement.itemId}` },
    warehouse: { name: warehouse?.nameAr || `Warehouse ${movement.warehouseId}` },
    project: {
      id: movement.project?.id ?? null,
      name: movement.project?.nameEn || movement.project?.nameAr || 'General'
    },
    document: {
      id: movement.id,
      type: movement.type,
      typeLabel: stockMovementLabel(movement.type),
      reference: movement.reference ?? `MOV-${movement.id}`,
      dateDisplay: dateLabel(movement.date),
      quantityDisplay: quantity(movement.quantity),
      unitCostDisplay: money(movement.unitCost),
      totalCostDisplay: money(movement.totalCost),
...
async function buildStockCountContext(recordId: number) {
  ...
  const [warehouse, lines] = await Promise.all([
    prisma.warehouse.findUnique({ where: { id: count.warehouseId } }),
    prisma.stockCountLine.findMany({ where: { stockCountId: count.id } })
  ]);
  ...
  return {
    ...
    company,
    branch: count.branch
      ? { id: count.branch.id, code: count.branch.code, nameAr: count.branch.nameAr }
      : { id: null, code: '', nameAr: 'Unassigned branch' },
    warehouse: { name: warehouse?.nameAr || `Warehouse ${count.warehouseId}` },
    document: {
      id: count.id,
      number: count.number,
      status: count.status,
      dateDisplay: dateLabel(count.date),
      notes: count.notes ?? ''
    },
    lines: lines.map((line) => ({
      itemName: itemMap.get(line.itemId)?.nameEn || itemMap.get(line.itemId)?.nameAr || `Item ${line.itemId}`,
      theoreticalQtyDisplay: quantity(line.theoreticalQty),
      actualQtyDisplay: quantity(line.actualQty),
      differenceQtyDisplay: quantity(line.differenceQty),
      unitCostDisplay: money(line.unitCost),
      differenceValueDisplay: money(line.differenceValue)
    })),
    generatedAt: dateTimeLabel(new Date())
  };
}

async function buildMaintenanceLogContext(recordId: number) {
  const [log, company] = await Promise.all([
    prisma.maintenanceLog.findUnique({
      where: { id: recordId },
      include: {
        branch: true,
        asset: true,
  ...
  if (!log) throw Errors.notFound('Maintenance log not found');
  ...
  return {
    ...
    company,
    branch: log.branch
      ? { id: log.branch.id, code: log.branch.code, nameAr: log.branch.nameAr }
      : { id: null, code: '', nameAr: 'Unassigned branch' },
    asset: { name: log.asset?.nameEn || log.asset?.nameAr || `Asset ${log.assetId}` },
    supplier: { name: log.supplier?.nameEn || log.supplier?.nameAr || 'Unassigned supplier' },
    project: { name: log.project?.nameEn || log.project?.nameAr || 'General' },
    document: {
      id: log.id,
      type: log.type,
      status: log.status,
      serviceDateDisplay: dateLabel(log.serviceDate),
      completedAtDisplay: dateLabel(log.completedAt),
      costDisplay: money(log.cost),
...
async function buildEntityContext(entityType: PrintingEntityType, recordId: number) {
  switch (entityType) {
    ...
    case 'payment':
      return buildPaymentContext(recordId);
    case 'journal_entry':
      return buildJournalEntryContext(recordId);
    case 'stock_movement':
      return buildStockMovementContext(recordId);
    case 'stock_count':
      return buildStockCountContext(recordId);
    case 'maintenance_log':
      return buildMaintenanceLogContext(recordId);
    default:
