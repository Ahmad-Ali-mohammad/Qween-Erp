...
describe('Central portal foundation', () => {
  ...
  it('exposes the system catalog and central health endpoints', async () => {
    ...
    expect(appsRes.body.data.some((row: { key: string; routeBase: string }) => row.key === 'control-center' && row.routeBase === '/portal')).toBe(true);
    expect(appsRes.body.data.some((row: { key: string; routeBase: string }) => row.key === 'accounting' && row.routeBase === '/systems/accounting')).toBe(true);
